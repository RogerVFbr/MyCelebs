...
class GetRegistrationLogs:

    @classmethod
    def get(cls, request_data):
        public_bucket_files = am.get_public_bucket_file_names()
        items = fd.fetch_data(ev.REGISTER_TABLE_NAME, request_data)
        return_list = []
        ...
        print(f'ADMIN - Retrieving {len(return_list)} items to client from register table. '
